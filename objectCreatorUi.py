...
class ObjectCreatorDialog(QtWidgets.QDialog):
    ...
    def onCreateClicked(self):
        ...
        if not item:
            QtWidgets.QMessageBox.warning(self, "⚠ Warning", "Please select an object type.")
            return

        obj_type = item.text()
        obj_name = self.name_lineEdit.text().strip()

        created = objectCreatorUtil.create_object(obj_type, obj_name)
